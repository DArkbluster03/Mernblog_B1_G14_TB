...
const editorJSHTML = EditorJSHTML(); // Initialize EditorJSHTML

const SingleBlogCard = ({ blog }) => {
  const { title, description, content, coverImg, category, rating, author, createdAt } = blog || {};
  const htmlContent = editorJSHTML.parse(content).join(''); // Parse content to HTML
      ...
      <div className='bg-white p-8'>
        ...
        <div className='mt-8 space-y-4'>
          ...
          <div>
            ...
            <span>{rating} (asjdaklsjdkasjfdokasj)</span>
          </div>
        </div>
      </div>
    </>
